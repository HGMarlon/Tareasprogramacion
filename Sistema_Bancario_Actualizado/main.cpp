...
void actualizarRegistro( fstream &actualizarArchivo )
{
   ...
   if ( cliente.obtenerNumeroCuenta() != 0 ) {
      ...
      // solicitar al usuario el nombre nuevo a ingresar
      cout << "\nEscriba el nuevo nombre: ";
      string newName; // nuevo nombre
      cin >> newName;

      // actualizar el nombre del registro
      string nombreAnterior = cliente.obtenerPrimerNombre();
      cliente.establecerPrimerNombre( newName );
      mostrarLinea( cout, cliente);

      // solicitar al usuario el apellido nuevo a ingresar
      cout << "\nEscriba el nuevo apellido: ";
      string newApellido; // nuevo apellido
      cin >> newApellido;
...
} // fin de la función actualizar Registro
